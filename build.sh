...
for f in js/script.js js/currency.js js/customize.js js/customize-data.js \
         ...
         js/portal.js js/portal-data.js js/prices.js js/site-prices.js js/admin.js \
         js/login.js js/portal-app.js; do
    [ -f "$f" ] || continue
    out="${f%.js}.min.js"
...
echo "CSS"
for f in css/style.css css/home.css css/faq.css css/social-calculator.css css/portal.css \
         css/admin.css css/login.css css/portal-app.css; do
    [ -f "$f" ] || continue
    out="${f%.css}.min.css"
    minify_css "$f" "$out"
    report "$f" "$out"
